...
export const AuthView: React.FC<AuthViewProps> = ({ mode = 'login' }) => {
  ...
  const handleGoogleAccountSelect = async (accountName: string, accountEmail: string, accountRole: 'organizer' | 'centre') => {
    setShowGoogleModal(false);
    setStatusMsg(null);

    const success = await registerUser(accountName, accountEmail, 'google_sso_pass', accountRole);
    if (success) {
      setStatusMsg({
        type: 'ok',
        text: `Signed in as ${accountName} via Google SSO! Redirecting…`,
      });
      setTimeout(() => {
        setActiveTab(accountRole === 'centre' ? 'centre-dash' : 'dashboard');
      }, 600);
    }
  };
    ...
    <section className="view active min-h-[85vh] flex items-center justify-center p-4 sm:p-6 relative" id="view-auth">
      ...
      {showGoogleModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-navy-900 border border-border/60 rounded-3xl p-6 space-y-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between border-b border-border/40 pb-4">
              <div className="flex items-center gap-3">
                ...
                <span className="font-heading font-bold text-foreground">Sign in with Google</span>
              </div>
              <button
                onClick={() => setShowGoogleModal(false)}
                className="p-1 rounded-xl text-muted-foreground hover:text-foreground hover:bg-card/60 cursor-pointer"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="space-y-2">
              <p className="text-xs text-muted-foreground">
                Choose a Google Account to continue to <strong className="text-foreground">Gatehouse Enterprise</strong>:
              </p>
              ...
              <div className="space-y-2.5 pt-2">
                ...
                <button
                  onClick={() => handleGoogleAccountSelect('Musa Ibrahim', 'musa.ibrahim@gmail.com', 'organizer')}
                  className="w-full p-3.5 rounded-2xl border border-border/60 bg-card/60 hover:bg-card hover:border-primary/40 text-left flex items-center justify-between transition-all cursor-pointer group"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-primary/20 text-primary font-bold font-mono flex items-center justify-center border border-primary/30">
                      MI
                    </div>
                    <div>
                      <div className="text-xs font-bold text-foreground group-hover:text-primary transition-colors">Musa Ibrahim</div>
                      <div className="text-[11px] font-mono text-muted-foreground">musa.ibrahim@gmail.com</div>
                    </div>
                  </div>
                ...
                {/* Account 2: Eko Hotels Security Manager */}
                <button
                  onClick={() => handleGoogleAccountSelect('Eko Hotels Security', 'security@ekohotels.com', 'centre')}
                  className="w-full p-3.5 rounded-2xl border border-border/60 bg-card/60 hover:bg-card hover:border-[#5cbdb9]/40 text-left flex items-center justify-between transition-all cursor-pointer group"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-[#5cbdb9]/20 text-[#5cbdb9] font-bold font-mono flex items-center justify-center border border-[#5cbdb9]/30">
                      EH
                    </div>
                    <div>
                      <div className="text-xs font-bold text-foreground group-hover:text-[#5cbdb9] transition-colors">Eko Hotels Management</div>
                      <div className="text-[11px] font-mono text-muted-foreground">security@ekohotels.com</div>
                    </div>
                  </div>
              ...
              <div className="pt-4 border-t border-border/40 space-y-3">
                <label className="text-[11px] font-mono text-muted-foreground font-bold">Or sign in with another Google account:</label>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 rounded-xl border border-border/80 bg-card px-3.5 py-2 text-xs text-foreground focus:border-primary focus:outline-none"
                  />
                  <button
                    ...
                    onClick={() => {
                      if (customGoogleEmail.includes('@')) {
                        ...
                        handleGoogleAccountSelect(prettyName, customGoogleEmail, role);
                      }
                    }}
                    className="px-4 py-2 rounded-xl bg-primary text-primary-foreground text-xs font-mono font-bold disabled:opacity-40 cursor-pointer"
                  >
                    Continue
                  </button>
                </div>
