...
export const AuthView: React.FC<{ mode?: "login" | "register" }> = ({
  mode = "login",
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (authMode === "login") {
      ...
      if (!name.trim()) {
        setStatusMsg({
          type: "err",
          text: "Full name or organization name is required.",
        });
        return;
      }
  ...
  const handleDemoLogin = async (targetRole: UserRole) => {
    ...
    const demoEmail =
      targetRole === "organizer"
        ? "chidinma@xquisitevents.ng"
        : "events@ekohotels.com";
      ...
      <div className="max-w-md mx-auto space-y-6 py-6 font-['Inter']">
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="text-2xl font-bold font-['Space_Grotesk'] text-[#EDEFF3]">
            {authMode === "login"
              ...
              : "Create Gatehouse Account"}
          </h2>
          <p className="text-xs text-[#8B93A3]">
            Access your event dashboards, venue bookings, and gate entry
            scanners.
          </p>
        </div>
        ...
        <div className="panel space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-mono text-[#8B93A3] uppercase">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 pt-2">
            ...
            <button
              type="submit"
              className="btn btn-go w-full py-3 text-xs font-bold shadow-lg shadow-[#3ED98A]/20"
            >
              {authMode === "login"
                ? "Sign In to Gatehouse"
