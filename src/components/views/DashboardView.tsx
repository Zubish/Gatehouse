...
export const DashboardView: React.FC = () => {
  const { guests, checkinTimeline, activeEvent, exportCsvReport } =
    useGatehouse();

  const [feedFilter, setFeedFilter] = useState<"all" | "VIP" | "Regular">(
    "all",
  );
  ...
  const attendanceRate =
    totalInvited > 0 ? Math.round((checkedInCount / totalInvited) * 100) : 0;
  const notArrivedCount = totalInvited - checkedInCount;

  // Category breakdown
  const vipCount = guests.filter((g) => g.category === "VIP").length;
  const vipCheckedIn = guests.filter(
    (g) => g.category === "VIP" && g.status === "in",
  ).length;
  const regularCount = guests.filter((g) => g.category === "Regular").length;
  const regularCheckedIn = guests.filter(
    (g) => g.category === "Regular" && g.status === "in",
  ).length;
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#262D38] pb-4">
          <div>
            ...
            <p className="text-xs font-mono text-[#8B93A3]">
              Event Date: {activeEvent.date} • Start Time:{" "}
              {activeEvent.startTime} • Capacity Cap: {activeEvent.capacity}
            </p>
          </div>
        ...
        <div className="stat-grid">
          <div className="stat-card">
            <div className="num" id="statInvited">
              {totalInvited}
            </div>
          ...
          <div className="stat-card go">
            <div className="num" id="statCheckedIn">
              {checkedInCount}
            </div>
          ...
          <div className="stat-card alert">
            <div className="num" id="statNotArrived">
              {notArrivedCount}
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="panel space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs font-mono">
                  ...
                  <span className="text-white">
                    {vipCheckedIn} / {vipCount} Checked In (
                    {vipCount > 0
                      ? Math.round((vipCheckedIn / vipCount) * 100)
                      : 0}
                    %)
                  </span>
                </div>
                <div className="h-3 bg-[#080c14] rounded-full overflow-hidden border border-[#262D38]">
                  <div
                    ...
                    style={{
                      width: `${vipCount > 0 ? (vipCheckedIn / vipCount) * 100 : 0}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs font-mono">
                  <span className="text-[#3ED98A] font-bold">
                    Regular Guests
                  </span>
                  <span className="text-white">
                    {regularCheckedIn} / {regularCount} Checked In (
                    {regularCount > 0
                      ? Math.round((regularCheckedIn / regularCount) * 100)
                      : 0}
                    %)
                  </span>
                </div>
                <div className="h-3 bg-[#080c14] rounded-full overflow-hidden border border-[#262D38]">
                  <div
                    ...
                    style={{
                      width: `${regularCount > 0 ? (regularCheckedIn / regularCount) * 100 : 0}%`,
                    }}
                  />
                </div>
              </div>
            ...
            <div className="h-36 bg-[#080c14] rounded-xl border border-[#262D38] p-4 flex items-end justify-between gap-2">
              {Object.keys(timeBuckets).length > 0 ? (
                Object.entries(timeBuckets).map(([timeStr, count]) => (
                  ...
                  >
                    <span className="text-[10px] font-mono text-[#3ED98A] font-bold">
                      {count}
                    </span>
                    <div
                    ...
                    />
                    <span className="text-[9px] font-mono text-[#8B93A3]">
                      {timeStr}
                    </span>
                  </div>
                ))
          ...
          <div className="feed" id="liveFeed">
            {feedList.length > 0 ? (
              feedList.slice(0, 10).map((g) => (
                <div
                  key={g.id}
                  className="feed-row flex items-center justify-between"
                >
                  <div className="flex items-center gap-3">
                    <div className="feed-dot" />
                    <span className="feed-name text-white font-bold">
                      {g.name}
                    </span>
                    <span
                      className={`tag text-[10px] ${g.category === "VIP" ? "tag-vip" : "tag-regular"}`}
                    >
                      {g.category}
                    </span>
                  ...
                  <span className="feed-time font-mono text-xs text-[#8B93A3]">
                    {g.checkinTime ? fmtTime(g.checkinTime) : "Just now"} •
                    Verified by {g.checkedInBy || "Gate Camera"}
                  </span>
                </div>
              ))
            ) : (
              <div className="empty" id="feedEmptyState">
                No check-in entries recorded yet. Access scanner to verify
                guests.
              </div>
            )}
          </div>
