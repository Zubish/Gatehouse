...
export const AdminPortalView: React.FC = () => {
  ...
  const [usersList, setUsersList] = useState([
    { id: 'u_org_1', name: 'Chidinma Okoro', email: 'chidinma@xquisit.ng', role: 'organizer', status: 'active', joined: 'Aug 2, 2026' },
    { id: 'u_centre_1', name: 'Eko Convention Centre', email: 'info@ekohotels.com', role: 'centre', status: 'active', joined: 'Jul 28, 2026' },
    { id: 'u_org_2', name: 'Babatunde Raji', email: 'baba@events.ng', role: 'organizer', status: 'active', joined: 'Aug 4, 2026' },
    { id: 'u_centre_2', name: 'Harbour Point VI', email: 'management@harbourpoint.ng', role: 'centre', status: 'active', joined: 'Jul 30, 2026' },
  ]);
    ...
    setUsersList((prev) =>
      prev.map((u) => {
        if (u.id === id) {
          ...
          setStatusMsg(`User ${u.name} status set to ${nextStatus.toUpperCase()}`);
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#262D38] pb-4">
          <div>
            ...
            <p className="text-xs font-mono text-[#8B93A3]">
              System-wide metrics, tenant accounts, platform audit logs &amp; enterprise subscription tiers.
            </p>
          </div>
        ...
        <div className="panel space-y-4">
          <div className="panel-head">
            ...
            <span className="text-xs font-mono text-[#8B93A3]">Super Admin Actions</span>
          </div>
            ...
            <tbody>
              {usersList.map((u) => (
                <tr key={u.id}>
                  ...
                  <td><span className="font-mono text-xs text-[#8B93A3]">{u.email}</span></td>
                  <td>
                    <span className={`tag text-[10px] ${u.role === 'centre' ? 'tag-vip' : 'tag-regular'}`}>
                      {u.role.toUpperCase()}
                    </span>
                  </td>
                  <td><span className="font-mono text-xs">{u.joined}</span></td>
                  <td>
                    <span className={`status-pill ${u.status === 'active' ? 'in' : 'out'}`}>
                      {u.status}
                    </span>
                  </td>
                  <td>
        ...
        <div className="panel space-y-4">
          <div className="panel-head">
            ...
            <span className="text-xs font-mono text-[#3ED98A]">Real-Time Security Feed</span>
          </div>

          <div className="space-y-2 text-xs font-mono">
            <div className="p-2.5 rounded-lg bg-[#080c14] border border-[#262D38] flex justify-between text-[#8B93A3]">
              <span>🔒 <strong>AUTH_JWT_SIGN:</strong> User chidinma@xquisit.ng signed in via JWT token session.</span>
              <span>2 mins ago</span>
            </div>
            <div className="p-2.5 rounded-lg bg-[#080c14] border border-[#262D38] flex justify-between text-[#8B93A3]">
              <span>🏢 <strong>VENUE_DELEGATION:</strong> Eko Convention Centre granted Path B delegation for EVT-8921.</span>
              <span>12 mins ago</span>
            </div>
            <div className="p-2.5 rounded-lg bg-[#080c14] border border-[#262D38] flex justify-between text-[#3ED98A]">
              <span>⚡ <strong>HMAC_SCAN_OK:</strong> Gate Camera Agent verified guest Babatunde Raji (EVT-TBK88).</span>
              <span>24 mins ago</span>
            </div>
            <div className="p-2.5 rounded-lg bg-[#080c14] border border-[#262D38] flex justify-between text-[#F0A93B]">
              <span>🔓 <strong>HARDWARE_RELAY:</strong> Turnstile pulse sent to Gate Lane #01 (3000ms duration).</span>
